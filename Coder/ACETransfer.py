...
class transferNASLearningEngin:
    ...
    # [architecture:string , parameter:float, error: float,...]
    def preLearning(self, knowledgeBase, knowledgeDimension=1):
            ...
            for component in componentMap:
                ...
                for metric in self.knowledgeMap:
                    if component not in self.knowledgeMap[metric].keys():
                        ...
                        self.knowledgeMap[metric][component][1] = 0.9*self.knowledgeMap[metric][component][1] + 0.1*(
                            gama[metric]*proportion-self.knowledgeMap[metric][component][0])
        self.calculateGeneralMetric()
        return True
    ...
    def getTop(self, number, order="INC"):
        ...
        for component, metric in self.generalMetric:
            componentList.append(
                np.array([np.int(element) for element in component.split(".")]))
    ...
    def calculateGeneralMetric(self):
        ...
        for metric in self.knowledgeMap:
            for component in self.knowledgeMap[metric]:
                ...
                generalMetricDic[component] += sum(
                    self.knowledgeMap[metric][component])

        for component in generalMetricDic:
            self.generalMetric.append(
                (component, generalMetricDic[component]/componentSize))
